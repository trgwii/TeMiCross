...
const { messageJSON, textJSON } = require("../client/generator");

const { code, escape, logError } = require("./utils");
...
const fromName = R.ifElse(
  fromLastName,
  R.o(joinSpace, R.converge(R.pair, [fromFirstName, fromLastName])),
  fromFirstName
);
...
const run = (opts) => {
  ...
  const send = (msg) =>
    bot.telegram
      .sendMessage(tgID, msg, tgOpts)
      .catch((err) => console.error("[TG.sendMessage]: ]" + err.message));
  ...
  client.on("goal", (msg) =>
    send(code(msg.user) + " has reached the goal " + code("[" + msg.goal + "]"))
  );
