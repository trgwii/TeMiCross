...
const surroundedBy = (a, b, str) =>
	str.startsWith(a) && str.endsWith(b);
...
const parse = str =>
	...
	surroundedBy('{', '}', str) ? bracketSplit(',', unwrap(str))
		...
		.reduce((acc, [ key, value ]) =>
			({ ...acc, [key]: value }), {}) :
	surroundedBy('"', '"', str) ? unwrap(str) :
	str.match(/^-?\d+L$/) ? str.slice(0, -1) :
	str.match(/^-?\d+(\.\d+)?(d|s|f|b)$/) ? Number(str.slice(0, -1)) :
